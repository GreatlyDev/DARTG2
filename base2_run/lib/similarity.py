...
def token_change_ratio(anchor: str, rewrite: str) -> float:
    """Fraction of anchor tokens that are NOT preserved in the rewrite.
    Used to check the 5%-25% surface-variation band from base2's prompt."""
    a, b = _tokens(anchor), _tokens(rewrite)
    if not a:
        return 0.0
    preserved = len(a & b)
    return round(1 - preserved / len(a), 4)
...
def cheap_scores(anchor: str, rewrite: str) -> dict:
    return {
        ...
        "token_change_ratio": token_change_ratio(anchor, rewrite),
